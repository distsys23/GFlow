...
class XMLtoDAG():
    ...
    def __init__(self, file):
        self.xmlFile = file
        filename = os.path.basename(file)  # 获取文件名，去掉路径部分
        self.n_task = int(re.compile(r'\d+').findall(filename)[0])
        self.folder_path = os.path.dirname(file)
        
        random.seed(seed)
        self.task_security_authentication = [random.random() for _ in range(self.n_task)]
        self.task_security_confidentialit = [random.random() for _ in range(self.n_task)]
        self.task_security_integrity = [random.random() for _ in range(self.n_task)]
        # self.xmlFile = file
    ...
    def get_edge_data(self):
        ...
        if job_1[0]['namespace'] == 'CyberShake':
            ...
            with open(f'{self.folder_path}/CyberShake_throughput.txt', 'r') as file: # # 打开文件
            ...
            with open(f'{self.folder_path}/Epigenomics_throughput.txt', 'r') as file: # # 打开文件
                text = file.read() # # 读取文件内容
            groups = text.split('\n\n') # 利用空行将连续的数字分组
            number_arrays = [] # 创建存储数字组的列表
            edges = np.zeros((self.n_task, self.n_task))
            ...
            with open(f'{self.folder_path}/Inspiral_throughput.txt', 'r') as file:
                text = file.read()
            groups = text.split('\n\n')
            number_arrays = [] 
            edges = np.zeros((self.n_task, self.n_task))
            ...
            with open(f'{self.folder_path}/Montage_throughput.txt', 'r') as file:
                text = file.read()
            groups = text.split('\n\n')
            number_arrays = [] 
            edges = np.zeros((self.n_task, self.n_task))
            ...
            with open(f'{self.folder_path}/Sipht_throughput.txt', 'r') as file:
                text = file.read()
            groups = text.split('\n\n')
            number_arrays = [] 
            edges = np.zeros((self.n_task, self.n_task))
    ...
    def jobs(self):
        ...
        for job in root.iter(tag=self.job_tag):
            ...
            job_index = int(pattern.findall(job.attrib['id'])[0])
            ...
            simple_job = {'id': int(pattern.findall(job.attrib['id'])[0]), 
                            ...
                            # 'security_authentication': random.random(),
                            # 'security_confidentiality': random.random(),
                            # 'security_integrity': random.random(),
                            'security_authentication': self.task_security_authentication[job_index],
                            'security_confidentiality': self.task_security_confidentialit[job_index],
                            'security_integrity': self.task_security_integrity[job_index],
